...
public class CropOverlayView extends View {
    ...
    /**
     * Get the start/top/right/bottom coordinates of the crop window.
     */
    ...
    /**
     * Set the start/top/right/bottom coordinates of the crop window.
     */
    ...
    private void initCropWindow() {
        ...
        float startLimit = Math.max(BitmapUtils.getRectLeft(mBoundsPoints), 0);
        ...
        if (rightLimit <= startLimit || bottomLimit <= topLimit) {
            return;
        }
        ...
        float horizontalPadding = mInitialCropWindowPaddingRatio * (rightLimit - startLimit);
        ...
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
            ...
            rect.left = startLimit + mInitialCropWindowRect.left / mCropWindowHandler.getScaleFactorWidth();
            ...
            if (bitmapAspectRatio > mTargetAspectRatio) {
                ...
                rect.left = startLimit + horizontalPadding;
    ...
    private void fixCropWindowRectByRules(RectF rect) {
        ...
        if (mCalcBounds.width() > 0 && mCalcBounds.height() > 0) {
            float startLimit = Math.max(mCalcBounds.left, 0);
            ...
            if (rect.left < startLimit) {
                rect.left = startLimit;
            }
    ...
    private void drawBackground(Canvas canvas) {
        ...
        float start = Math.max(BitmapUtils.getRectLeft(mBoundsPoints), 0);
        ...
        if (mCropShape == CropImageView.CropShape.RECTANGLE) {
            if (!isNonStraightAngleRotated() || Build.VERSION.SDK_INT <= 17) {
                canvas.drawRect(start, top, right, rect.top, mBackgroundPaint);
                canvas.drawRect(start, rect.bottom, right, bottom, mBackgroundPaint);
                canvas.drawRect(start, rect.top, rect.left, rect.bottom, mBackgroundPaint);
                ...
                canvas.drawRect(start, top, right, bottom, mBackgroundPaint);
                canvas.restore();
            }
        } else {
            mPath.reset();
    ...
    /**
     * On press down left crop window movment deprighting on the location of the press.<br>
    ...
    /**
     * Clear move handler lefting in {@link #onActionDown(float, float)} if exists.
     */
     ...
     * otherwsie we find the max rectangle that is within the image bounds lefting from the crop window rectangle.
     *
     * @param rect the crop window rectangle to left finsing bounded rectangle from
