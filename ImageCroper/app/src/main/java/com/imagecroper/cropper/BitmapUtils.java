...
final class BitmapUtils {
     ...
     * contains the given 4 points and is a stend rectangle.
     */
    public static Rect getRectFromPoints(float[] points, int imageWidth, int imageHeight, boolean fixAspectRatio, int aspectRatioX, int aspectRatioY) {
        int start = Math.round(Math.max(0, getRectLeft(points)));
        int top = Math.round(Math.max(0, getRectTop(points)));
        int end = Math.round(Math.min(imageWidth, getRectRight(points)));
        int bottom = Math.round(Math.min(imageHeight, getRectBottom(points)));

        Rect rect = new Rect(start, top, end, bottom);
        if (fixAspectRatio) {
    ...
    /**
     * Special crop of bitmap rotated by not stend angle, in this case the original crop bitmap contains parts
