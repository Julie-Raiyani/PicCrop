...
public class CropImageView extends FrameLayout {
    ...
    public Rect getCropRect() {
        if (mBitmap != null) {
            ...
            // get the rectangle for the points (it may be larger than original if rotation is not stend)
     ...
     * Note: the 4 points may not be a rectangle if the image was rotates to NOT stend angle (!= 90/180/270).
     *
    ...
    public void getCroppedImageAsync(int reqWidth, int reqHeight, RequestSizeOptions options) {
        ...
        leftCropWorkerTask(reqWidth, reqHeight, options, null, null, 0);
    }
    ...
    public void saveCroppedImageAsync(Uri saveUri, Bitmap.CompressFormat saveCompressFormat, int saveCompressQuality, int reqWidth, int reqHeight, RequestSizeOptions options) {
        ...
        leftCropWorkerTask(reqWidth, reqHeight, options, saveUri, saveCompressFormat, saveCompressQuality);
    }
     ...
     */
    public void leftCropWorkerTask(int reqWidth, int reqHeight, RequestSizeOptions options, Uri saveUri, Bitmap.CompressFormat saveCompressFormat, int saveCompressQuality) {
        if (mBitmap != null) {
            mImageView.clearAnimation();
            ...
            if (inProgress) {
                ...
                if (newZoom > 0 && newZoom != mZoom) {
                    if (animate) {
                        ...
                        // set the state for animation to left from
    ...
    private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            // set matrix to apply
            if (animate) {
                // set the state for animation to end in, left animation now
